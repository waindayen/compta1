...
import ManagerApprovalAccessNav from '../../../components/admin/manager/ManagerApprovalAccessNav';
import ManagerApprovalAccessStats from '../../../components/admin/manager/ManagerApprovalAccessStats';
...
export default function ManagerApprovalAccess() {
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        users.push({
          ...
          canApprove: userData.canApprove !== false // true par défaut
        });
      });
  ...
  const toggleApprovalAccess = async (userId: string, canApprove: boolean) => {
    try {
  ...
  // Statistiques
  const enabledCount = managers.filter(m => m.canApprove).length;
  const disabledCount = managers.length - enabledCount;

  if (loading) {
    return (
      <BaseDashboard title="Accès aux Approbations pour les Managers">
  ...
  return (
    <BaseDashboard title="Accès aux Approbations pour les Managers">
      <ManagerApprovalAccessNav />
      
      <ManagerApprovalAccessStats 
      ...
      <div className="space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-blue-800">
            Activez ou désactivez la possibilité pour les managers d'approuver ou rejeter les demandes de calcul de gains.
            Lorsque cette option est désactivée, les boutons "Approuver" et "Rejeter" n'apparaîtront pas pour le manager.
          </p>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right text-sm font-medium text-gray-500">Accès aux Approbations</th>
                </tr>
              </thead>
                  ...
                  filteredManagers.map((manager) => (
                    <tr key={manager.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-right">
                        <button
                          onClick={() => toggleApprovalAccess(manager.id, !manager.canApprove)}

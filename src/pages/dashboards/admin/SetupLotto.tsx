...
import RecurrenceScheduler from '../../../components/admin/lotto/RecurrenceScheduler';
...
export default function SetupLotto() {
  ...
  const validateDates = () => {
    const startDate = useCurrentDate 
      ? new Date() 
      : new Date(formData.startDate + (formData.startDate.includes('T') ? '' : 'T00:00:00') + 'Z');
    const endDate = new Date(formData.endDate + (formData.endDate.includes('T') ? '' : 'T00:00:00') + 'Z');
    const now = new Date();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const lottoData = {
        ...
        endDate: new Date(formData.endDate + (formData.endDate.includes('T') ? '' : 'T00:00:00') + 'Z').toISOString(),
      ...
      if (id) {
        ...
        // Si c'est un modèle récurrent, rediriger vers la page des modèles
        if (formData.frequency !== 'once') {
          navigate('/dashboard/admin/lotto-templates');
        } else {
    ...
    <BaseDashboard title={id ? "Modifier le Lotto" : "Créer un Lotto"}>
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Programmation de récurrence */}
          {formData.frequency !== 'once' && formData.startDate && formData.endDate && (
            <RecurrenceScheduler 

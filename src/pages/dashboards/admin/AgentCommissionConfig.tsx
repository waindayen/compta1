...
export default function AgentCommissionConfig() {
  ...
  const toggleCommissionConversion = async (userId: string, canConvert: boolean) => {
    try {
      ...
      setAgentUsers(prev => prev.map(user => 
        user.id === userId 
          ? { ...user, canConvertCommission: canConvert }
          : user
      ));
    } catch (err) {
      console.error('Error updating commission conversion:', err);
      setError('Erreur lors de la mise à jour de la conversion de commission');
    } finally {

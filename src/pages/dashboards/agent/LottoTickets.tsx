...
export default function LottoTickets() {
  ...
  const displayedTickets = React.useMemo(() => {
    if (!ticketNumber.trim()) {
      // Sans recherche, afficher uniquement les tickets en attente de l'utilisateur actuel
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const tomorrow = new Date(today);
      tomorrow.setDate(tomorrow.getDate() + 1);
      
      // Filtrer uniquement les tickets en attente de l'utilisateur actuel
      ...
      // Combiner : tous les tickets du jour + les 5 plus récents des autres jours
      const finalPendingTickets = [...sortedTodayPendingTickets, ...sortedOtherPendingTickets.slice(0, Math.max(0, 15 - sortedTodayPendingTickets.length))];
  ...
  }, [allTickets, ticketNumber, currentUser]);
  ...
  const handleSearch = async (searchTerm: string) => {
    ...
    if (searchTerm.trim()) {
      ...
      // Si pas de recherche, recharger les tickets de l'utilisateur

...
export class LottoService {
  static async createLotto(data: Omit<LottoEvent, 'id' | 'createdAt' | 'status'>): Promise<string> {
    try {
      ...
      // Vérifier si la date de début est dans le passé, utiliser la date actuelle si c'est le cas
      if (startDate < now) {
      ...
      const lottoData = {
        ...data,
        startDate: startDate.toISOString(), // Utiliser la date de début ajustée

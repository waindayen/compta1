...
import { DailyRevenue, DailyAccountingService } from '../../../services/manager/dailyAccounting';
...
const REVENUE_CATEGORIES = [
  ...
  { value: 'bonuses', label: 'Bonus' },
  { value: 'other', label: 'Autres' }
];

export function RevenueForm({ managerId, revenue, onSuccess, onCancel }: RevenueFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!revenue?.id) {
        revenueData.performedBy = currentUser?.uid || '';
        revenueData.performedByName = getDisplayName(userData);
      }

      if (revenue?.id) {
        await DailyAccountingService.updateRevenue(revenue.id, revenueData);
      } else {
        await DailyAccountingService.createRevenue(revenueData);
      }
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-2xl w-full">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3 pt-4">
          ...
          >
            {loading ? 'Enregistrement...' : revenue ? 'Modifier' : 'Enregistrer'}
          </button>
        </div>
      </form>
    </div>
